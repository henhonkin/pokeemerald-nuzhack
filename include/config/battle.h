...
#define B_FLAG_NO_BAG_USE           FLAG_UNUSED_0x264     // If this flag is set, the ability to use the bag in battle is disabled.
...
#define B_NATURE_POWER_MOVES        GEN_LATEST // Nature Power calls different moves depending on terrain and generation. See sNaturePowerMoves. @HU_TODO Hydropump -> Surf (so that it's always accurate)
...
#define B_WAIT_TIME_MULTIPLIER      16    // This determines how long text pauses in battle last. Vanilla is 16. Lower values result in faster battles. @HU_TODO experiment
...
#define B_AFFECTION_MECHANICS           GEN_LATEST       // In Gen6+, there's a stat called affection that can trigger different effects in battle. From LGPE onwards, those effects use friendship instead.
